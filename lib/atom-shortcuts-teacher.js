...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.commands.onDidDispatch(handleCommand));
  },
...
function handleCommand(event) {
  ...
  const description = `You can run ${command} using ${describeBindings(keybindings)}`;
  atom.notifications.addInfo(description, { dismissable: false });
}

/**
 * Takes an array of KeyBinding objects and returns a string describing these.
 *
 * @param {KeyBinding[]} bindingsArray the keybindings to list
 *
 * @return {string} A description like "cmd-Q or ctrl-shift-Q"
 */
function describeBindings(bindingsArray) {
  var descriptionsArray = [];
  for (let binding of bindingsArray) {
    descriptionsArray.push(describeKeystrokeArray(binding.keystrokeArray));
  }
 ...
 * @return {string} A description of the keystroke sequence
 */
function describeKeystrokeArray(keystrokeArray) {
  return keystrokeArray.join(' ');
}
